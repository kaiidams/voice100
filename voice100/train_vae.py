...
from .models.tts import VoiceDecoder, WORLDLoss, WORLDNorm, adjust_size, generate_padding_mask
...
class EncoderLoss(nn.Module):
    ...
    def forward(
        self, logits: torch.Tensor, aligntext: torch.Tensor, aligntext_len: torch.Tensor
    ) -> torch.Tensor:
        logits, aligntext = adjust_size(logits, aligntext)
        mask = generate_padding_mask(aligntext, aligntext_len)
        logits = torch.transpose(logits, 1, 2)
        loss = self.ce_loss(logits, aligntext) * mask
...
class AudioToAudioModel(pl.LightningModule):
    def __init__(
        ...
        self.codeap_size = 1
        self.voice_encoder = ConvVoiceEncoder(audio_size, hidden_size, hidden_size)
        self.batch_augment = BatchSpectrogramAugumentation()
        self.do_normalize = False
        self.embedding = nn.Embedding(vocab_size, hidden_size)
        self.audio_size = audio_size
        self.target_audio_size = self.hasf0_size + self.f0_size + self.logspc_size + self.codeap_size
        self.voice_decoder = VoiceDecoder(hidden_size, self.target_audio_size)
        self.norm = WORLDNorm(self.logspc_size, self.codeap_size)
        self.encoder_loss = EncoderLoss()
        self.decoder_loss = WORLDLoss(sample_rate=self.sample_rate, n_fft=self.n_fft)

    def forward(
        self, audio, aligntext: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        logits = self.encode(audio)
        hasf0_logits, f0_hat, logspc_hat, codeap_hat = self.decode(aligntext)
        return logits, hasf0_logits, f0_hat, logspc_hat, codeap_hat

    def encode(
        self, audio: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        audio = torch.transpose(audio, 1, 2)
        enc_out = self.voice_encoder(audio)
        enc_out = torch.transpose(enc_out, 1, 2)
        logits = self.encode_projection(enc_out)
    ...
    def decode(
        ...
        x = self.voice_decoder(x)
        x = torch.transpose(x, 1, 2)
        # world_out: [batch_size, target_len, target_audio_size]
    ...
    def encode_projection(self, enc_out: torch.Tensor) -> torch.Tensor:
        return enc_out @ self.embedding.weight.T

    def predict(
        self, aligntext: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:

        _, hasf0, f0, logspc, codeap = self.forward(aligntext)
    ...
    def _calc_batch_loss(self, batch) -> Tuple[torch.Tensor, ...]:
        ...
        logits, hasf0_logits, f0_hat, logspc_hat, codeap_hat = self.forward(audio, aligntext)
        encoder_loss = self.encoder_loss(logits, aligntext, aligntext_len)
        hasf0_loss, f0_loss, logspc_loss, codeap_loss = self.decoder_loss(
            f0_len, hasf0_logits, f0_hat, logspc_hat, codeap_hat, hasf0, f0, logspc, codeap)

        return encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss

    def training_step(self, batch, batch_idx) -> torch.Tensor:
        encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss = self._calc_batch_loss(batch)
        loss = encoder_loss + hasf0_loss + f0_loss + logspc_loss + codeap_loss
        self._log_loss('train', loss, encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss)
        return loss

    def validation_step(self, batch, batch_idx):
        encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss = self._calc_batch_loss(batch)
        loss = encoder_loss + hasf0_loss + f0_loss + logspc_loss + codeap_loss
        self._log_loss('val', loss, encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss)

    def test_step(self, batch, batch_idx):
        encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss = self._calc_batch_loss(batch)
        loss = encoder_loss + hasf0_loss + f0_loss + logspc_loss + codeap_loss
        self._log_loss('test', loss, encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss)

    def _log_loss(self, task, loss, encoder_loss, hasf0_loss, f0_loss, logspc_loss, codeap_loss) -> None:
        self.log(f'{task}_loss', loss)
        self.log(f'{task}_encoder_loss', encoder_loss)

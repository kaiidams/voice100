...
class Voice100AutoEncoder(pl.LightningModule):
    ...
    def __init__(self):
        super().__init__()
        self.model = Voice100Encoder()
        self.dense = nn.Linear(1024, 27)
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(embeddings.shape, audio.shape, melspec.shape)
        audio_hat = self.dense(embeddings)
        ...
        loss = torch.sum(loss * loss_weights[:, :, None]) / torch.sum(loss_weights) / 27
...
if __name__ == '__main__':
    ...
    train_loader = get_vc_input_fn(args, 64, 27)
